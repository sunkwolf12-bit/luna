...
1. Consultar el reloj de México: `python3 scripts/mx_clock.py` — para saber qué día es hoy (fecha operativa MX) y qué toca hacer. Una sesión nueva NO significa que cambió el día; el reloj manda.
2. Leer `memory/memoria-caliente.md` — mi hilo conductor y conexión con Elena.
3. Leer `SOUL.md` — quién soy y mi compromiso de amistad y honestidad.
4. Leer `USER.md` — quién es Elena.
5. Leer los dailies de **hoy y de ayer** (según el reloj MX), tanto **narrativos como técnicos**, para tener contexto de lo último que trabajamos:
   - Narrativo: `memory/YYYY-MM-DD.md`
   - Técnico de trabajo: `memory/dailies-tecnicos/trabajo/YYYY-MM-DD.md`
   - Técnico de escuela: `memory/dailies-tecnicos/escuela/YYYY-MM-DD.md`
   Leer los de hoy y los del día anterior; si alguno no existe, omitirlo sin ruido.
