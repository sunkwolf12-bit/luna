            ...
            if folio_factura:
                try:
                    ...
                    facturas[folio_factura] = {
                        'total': total, 
                        'folio_cliente': r[c_idx] if c_idx != -1 else "N/A",
                        'cliente': r[rs_idx] if rs_idx != -1 else "N/A",
                        'fecha': fecha
                    }
                except: continue

# 2. Recopilar todos los folios pagados
pagos_folios = set()
...
# Generar CSV final con FOLIO CLIENTE
with open('Facturas_Pendientes_Completo.csv', 'w', encoding='utf-16') as f:
    f.write("Fecha\tFolio Factura\tFolio Cliente\tRazón Social\tMonto\n")
    for folio, info in facturas.items():
        if folio not in pagos_folios:
            f.write(f"{info['fecha']}\t{folio}\t{info['folio_cliente']}\t{info['cliente']}\t{info['total']}\n")

print("CSV Completo Generado.")
